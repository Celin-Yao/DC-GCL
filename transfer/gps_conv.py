...
class GPSConv(torch.nn.Module):
    ...
    def __init__(
        ...
        self.attn_drophead = attn_drophead
        ...
        self.norm1 = BatchNorm(channels) if norm == 'batch_norm' else None
        self.norm2 = BatchNorm(channels) if norm == 'batch_norm' else None
        self.norm3 = BatchNorm(channels) if norm == 'batch_norm' else None
    ...
    def forward(
        ...
        **kwargs,
        ...
        num_heads = int(self.heads * self.attn_drophead)
        # 随机把 num_heads 个head的权重置为0
        if num_heads > 0:
            h = h.reshape(-1, self.heads, h.size(-2), h.size(-1) // self.heads)
            perm = torch.randperm(self.heads)
            h[:, perm[:num_heads]] = 0
            h = h.reshape(-1, h.size(-2), h.size(-1) * self.heads)
        h = h[mask]
        h = F.dropout(h, p=self.dropout, training=self.training)
