...
class MoleculeDataset_aug(InMemoryDataset):
    ...
    def get(self, idx):
        ...
        elif self.aug == 'PEMask':
            data = PE_mask(data, self.aug_ratio)
...
def PE_mask(data, aug_ratio):
    pe = data.pe
    mask_num = int(pe.size(1) * aug_ratio)
...
def gumbel_softmax(logits, rate, tau=1, hard=True, dim=-1):
    def _gen_gumbels():
...
def Token_mask(data, aug_ratio):
    ...
    node_mask = gumbel_softmax(node_score, rate=1 - aug_ratio, tau=1, hard=True)
    node_mask[:, 0] = 1.
...
def mask_nodes(data, aug_ratio):
    ...
    data.x[idx_mask] = torch.tensor(0, dtype=torch.long)
