...
def train(args, model, device, loader, optimizer):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        pred = model(batch.x, batch.pe, batch.edge_index, batch.edge_attr, batch.batch)
...
def main():
    ...
    dataset = addPe(dataset, transform)
    ...
    if args.dataset in 'clintox':
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=40, gamma=0.5)
    elif args.dataset in 'bbbp':
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=70, gamma=0.25)
    else:
        scheduler = None
    ...
    with open('result.log', 'a+') as f:
        f.write(args.dataset + ' ' + str(args.runseed) + ' ' + str(np.array(best_test)))
        f.write('\n')
